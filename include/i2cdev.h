/**
 * @file i2cdev.h
 * @defgroup i2cdev i2cdev
 **/
#ifndef __I2CDEV_H__
#define __I2CDEV_H__

#include <zephyr/kernel.h>
#include <zephyr/drivers/i2c.h>
#include <zephyr/logging/log.h>

#ifdef __cplusplus
extern "C" {
#endif

LOG_MODULE_REGISTER(i2cdev);

#define I2CDEV_MAX_STRETCH_TIME K_MSEC(1000)  // Define an appropriate timeout

/**
 * I2C device descriptor
 */
typedef struct
{
    const struct device *dev;  //!< I2C device instance
    struct i2c_config cfg;     //!< I2C driver configuration
    uint8_t addr;              //!< Device address
    k_mutex mutex;             //!< Device mutex
    k_timeout_t timeout;       //!< Timeout for I2C operations
} i2c_dev_t;

/**
 * @brief Init library
 *
 * The function must be called before any other
 * functions of this library.
 *
 * @return 0 on success
 */
int i2cdev_init(void);

/**
 * @brief Finish work with library
 *
 * Uninstall I2C drivers.
 *
 * @return 0 on success
 */
int i2cdev_done(void);

/**
 * @brief Create mutex for device descriptor
 *
 * @param dev Device descriptor
 * @return 0 on success
 */
int i2c_dev_create_mutex(i2c_dev_t *dev);

/**
 * @brief Delete mutex for device descriptor
 *
 * @param dev Device descriptor
 * @return 0 on success
 */
int i2c_dev_delete_mutex(i2c_dev_t *dev);

/**
 * @brief Take device mutex
 *
 * @param dev Device descriptor
 * @return 0 on success
 */
int i2c_dev_take_mutex(i2c_dev_t *dev);

/**
 * @brief Give device mutex
 *
 * @param dev Device descriptor
 * @return 0 on success
 */
int i2c_dev_give_mutex(i2c_dev_t *dev);

/**
 * @brief Read from slave device
 *
 * Issue a send operation of \p out_data register address, followed by reading \p in_size bytes
 * from slave into \p in_data.
 * Function is thread-safe.
 *
 * @param dev Device descriptor
 * @param out_data Pointer to data to send if non-null
 * @param out_size Size of data to send
 * @param[out] in_data Pointer to input data buffer
 * @param in_size Number of bytes to read
 * @return 0 on success
 */
int i2c_dev_read(const i2c_dev_t *dev, const void *out_data,
        size_t out_size, void *in_data, size_t in_size);

/**
 * @brief Write to slave device
 *
 * Write \p out_size bytes from \p out_data to slave at \p out_reg register address.
 * Function is thread-safe.
 *
 * @param dev Device descriptor
 * @param out_reg Pointer to register address to send if non-null
 * @param out_reg_size Size of register address
 * @param out_data Pointer to data to send
 * @param out_size Size of data to send
 * @return 0 on success
 */
int i2c_dev_write(const i2c_dev_t *dev, const void *out_reg,
        size_t out_reg_size, const void *out_data, size_t out_size);

/**
 * @brief Read from register with an 8-bit address
 *
 * Shortcut to ::i2c_dev_read().
 *
 * @param dev Device descriptor
 * @param reg Register address
 * @param[out] in_data Pointer to input data buffer
 * @param in_size Number of bytes to read
 * @return 0 on success
 */
int i2c_dev_read_reg(const i2c_dev_t *dev, uint8_t reg,
        void *in_data, size_t in_size);

/**
 * @brief Write to register with an 8-bit address
 *
 * Shortcut to ::i2c_dev_write().
 *
 * @param dev Device descriptor
 * @param reg Register address
 * @param out_data Pointer to data to send
 * @param out_size Size of data to send
 * @return 0 on success
 */
int i2c_dev_write_reg(const i2c_dev_t *dev, uint8_t reg,
        const void *out_data, size_t out_size);

#define I2C_DEV_TAKE_MUTEX(dev) do { \
        int __ = i2c_dev_take_mutex(dev); \
        if (__ != 0) return __;\
    } while (0)

#define I2C_DEV_GIVE_MUTEX(dev) do { \
        int __ = i2c_dev_give_mutex(dev); \
        if (__ != 0) return __;\
    } while (0)

#define I2C_DEV_CHECK(dev, X) do { \
        int ___ = X; \
        if (___ != 0) { \
            I2C_DEV_GIVE_MUTEX(dev); \
            return ___; \
        } \
    } while (0)

#define I2C_DEV_CHECK_LOGE(dev, X, msg, ...) do { \
        int ___ = X; \
        if (___ != 0) { \
            I2C_DEV_GIVE_MUTEX(dev); \
            LOG_ERR(msg, ## __VA_ARGS__); \
            return ___; \
        } \
    } while (0)

#ifdef __cplusplus
}
#endif

/**@}*/

#endif /* __I2CDEV_H__ */
